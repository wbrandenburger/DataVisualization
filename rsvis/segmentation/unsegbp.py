...
#   function ----------------------------------------------------------------
# ---------------------------------------------------------------------------
def unsegbp(parent, img_list, logger, dim1=32, dim2=64, min_label=4, max_label=256, iterCount=64, factor=1, net=1, height=False, segments="xx"):

    #   settings ------------------------------------------------------------
    mtime = MTime(number=iterCount)

    new_shape = (math.ceil(img_list[0].shape[0]), math.ceil(img_list[0].shape[1]))
    img = cv2.resize(img_list[0], (new_shape[1], new_shape[0]), dst=cv2.CV_8UC3, interpolation=cv2.INTER_LINEAR)
    ...
    seg_map = cv2.resize(img_list[1],  (new_shape[1], new_shape[0]), dst=cv2.CV_32SC1, interpolation=cv2.INTER_NEAREST)
    seg_map = seg_map.flatten() # vector with shape (height*width,)
    ...
    for batch_idx, mt in zip(range(iterCount), mtime):
        ...
        seg_out = seg_out.reshape(new_shape)
        seg_out_refined = seg_out_refined.reshape(new_shape)
        ...
        seg_out_color = color.label2rgb(seg_out, img, kind='avg', bg_label=0)
        seg_out_color_refined = color.label2rgb(seg_out_refined, img, kind='avg', bg_label=0)
    ...
    for idx, imgs in enumerate(img_list):
        cv2.imwrite("B:\Temp\images\seg_{}_{}_{}_{}_{}_{}.png".format(factor, iterCount, segments, dim1, dim2, idx), imgs)
