...
class ImgSeg:
    ...
    def get_seg_map_boundaries(self):
        img = self.get_seg_map_color()
        if self._boundaries=="mark":
            return segmentation.mark_boundaries(img, self._seg_map, mode="subpixel")
        elif self._boundaries=="find":
            return segmentation.find_boundaries(img, self._seg_map, mode="subpixel")
